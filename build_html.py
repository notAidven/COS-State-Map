...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* ── Legend (sits above the map, always visible) ── */
    .map-legend {
      display: flex; flex-wrap: wrap; align-items: stretch; gap: 8px 18px;
    ...
    .map-legend .legend-title {
      ...
      letter-spacing: .07em; color: var(--slate-500);
      align-self: center; padding-right: 18px;
      border-right: 1px solid var(--slate-200); white-space: nowrap;
    }
    .legend-item { display: flex; align-items: flex-start; gap: 8px;
                   flex: 1 1 190px; min-width: 175px; }
    .legend-swatch { width: 13px; height: 13px; border-radius: 3px; flex-shrink: 0;
                     margin-top: 3px; border: 1px solid rgba(0,0,0,.12); }
    .legend-label { font-size: 0.78rem; font-weight: 600; color: var(--slate-700);
                    line-height: 1.3; }
    .legend-desc  { font-size: 0.69rem; color: var(--slate-500); line-height: 1.45;
                    margin-top: 2px; }
    @media (max-width: 760px) {
      .map-legend .legend-title { border-right: none; padding-right: 0; flex: 1 1 100%; }
    }
...
<div class="container">
  ...
  <div class="map-legend">
    <div class="legend-title">Program<br>Status</div>
    <div class="legend-item">
      <div class="legend-swatch" style="background:#0f766e"></div>
      <div><div class="legend-label">Active Program</div>
        <div class="legend-desc">Community solar is enabled in law or regulation &mdash; subscribers share a project and receive bill credits.</div></div>
    </div>
    <div class="legend-item">
      <div class="legend-swatch" style="background:#14b8a6"></div>
      <div><div class="legend-label">Limited / Emerging</div>
        <div class="legend-desc">No full statewide program, but partial building blocks: virtual net metering, a capped pilot, or utility / co-op shared solar.</div></div>
    </div>
    <div class="legend-item">
      <div class="legend-swatch" style="background:#99f6e4"></div>
      <div><div class="legend-label">No Program</div>
        <div class="legend-desc">No community solar policy and no virtual net metering; nothing enabling shared ownership is on the books.</div></div>
    </div>
    <div class="legend-item">
      <div class="legend-swatch" style="background:#94a3b8"></div>
      <div><div class="legend-label">No Data</div>
        <div class="legend-desc">Not covered by the report &mdash; status has not been assessed.</div></div>
    </div>
  </div>
...
function showWelcome() {
  ...
  document.getElementById('panel-content').innerHTML =
    ...
    + 'Washington DC, and the US territories. Each state is shaded by how far its policy framework has developed. '
    + 'Click any state to read its detailed policy report.</p>'
    + '<div class="fact-title" style="margin-top:18px">What the three statuses mean</div>'
    + '<div class="status-guide">'
    + statusCard('active', 'Active Program', na,
        'The state has <strong>enabled community solar in law or regulation</strong>. A statute or '
      + 'regulator-approved program lets multiple customers subscribe to a shared solar project and receive '
      + 'credits on their utility bills, so participation does not require owning a suitable roof.')
    + statusCard('limited', 'Limited / Emerging', nl,
        'No full statewide community solar program, but <strong>some of the building blocks are in place</strong>. '
      + 'These states typically allow virtual or remote net metering, run a capped or time-limited pilot, or rely on '
      + 'voluntary utility and rural-cooperative shared solar offerings. Participation is possible but constrained '
      + 'by size caps, eligibility rules, or the absence of a permanent framework.')
    + statusCard('none', 'No Program', nn,
        'The state has <strong>no community solar policy and no virtual net metering</strong>. Without a mechanism '
      + 'to move credits from a shared project to a subscriber\'s bill, there is no enabling pathway for shared or '
      + 'community-owned solar, though legislation may be pending.')
    + '</div>'
    + '<p class="welcome-body" style="margin-top:14px;font-size:0.73rem;color:var(--slate-500)">'
    + 'Status is assigned from each state\'s Community Solar and Virtual / Remote Net Metering findings, plus any '
    + 'documented program size, eligibility, and benefit-distribution rules. Open a state and expand '
    + '&ldquo;Policies at a glance&rdquo; to see the evidence behind its shading.</p>'
    + '</div>';
